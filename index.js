...
async function fetchData(retry, artStr) {
    ...
    return await fetch(
        ...
        .then((receivedJson) => {
            ...
            if (conting) {
                if (document.getElementById("See_also") != null) {
                    ...
                    var seeAlso = document.getElementById("References")
                        .parentNode;
                }
        ...
        .catch((err) => {
            ...
            alert(
                "Something went wrong while loading the page. Try refreshing."
            );
        });
}
LoadSave();
        ...
        if (!alreadyGuessed || populate) {
            ...
            for (var i = 0; i < baffled.length; i++) {
                if (baffled[i][0] == normGuess) {
                    baffled[i][1].reveal();
                    baffled[i][1].elements[0].element.classList.remove(
                        "baffled"
                    );
                    ...
                    numHits += 1;
                    if (!populate) {
                        baffled[i][1].elements[0].element.classList.add(
                            "highlighted"
                        );
    ...
    if (guess[1] > 0) {
        $(newRow).on("click", function(e) {
            ...
            var inTxt = this.getElementsByTagName("td")[1]
                ...
                .replace(/[\u0300-\u036f]/g, "")
                .toLowerCase();
            allInstances = wikiHolder.querySelectorAll(
                '[data-word="' + inTxt + '"]'
            );
